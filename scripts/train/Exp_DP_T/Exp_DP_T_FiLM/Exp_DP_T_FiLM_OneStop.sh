...
EXP_NAME="Exp_DP_T_FiLM_OneStop"
# build your run_dir

# ---
# Iterate through each letter and run the corresponding task
# ---
run_name_stage1="${EXP_NAME}__${INPUT_TASK_LETTERS}_stage1_self_attn"
run_name_stage2="${EXP_NAME}__${INPUT_TASK_LETTERS}_stage2_self_attn"
...
python trainer_pl_all.py \
    ...
    \
    policy.enable_film_self_attn=True \
    policy.enable_film_ffn=False \
    \
    ...
    python trainer_pl_all.py \
        --config-name=DP_DecoupleActionHead_transformer_stage2_film \
        \
        policy.enable_film_self_attn=True \
        policy.enable_film_ffn=False \
        \
...
run_name_stage1="${EXP_NAME}__${INPUT_TASK_LETTERS}_stage1_ffn"
run_name_stage2="${EXP_NAME}__${INPUT_TASK_LETTERS}_stage2_ffn"
run_dir_stage1="data/outputs/${date_part}/${time_part}_${run_name_stage1}"
run_dir_stage2="data/outputs/${date_part}/${time_part}_${run_name_stage2}"
...
python trainer_pl_all.py \
    --config-name=DP_DecoupleActionHead_transformer_stage1_film \
    \
    policy.enable_film_self_attn=False \
    policy.enable_film_ffn=True \
    \
    task_alphabet=$INPUT_TASK_LETTERS \
    train_mode=stage1 \
    n_demo=1000 \
