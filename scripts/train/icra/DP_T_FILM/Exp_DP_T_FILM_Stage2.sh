...
EXP_NAME="ICRA_DP_T_FILM_Stage2_seed${SEED}"
...
for LETTER in $(echo "$INPUT_TASK_LETTERS" | sed -e 's/\(.\)/\1 /g'); do
    ...
    ckpt_path="data/robomimic/Stage1/ICRA_DP_T_FILM_Stage1_Multi__ABCDEFGH_epoch\=005.ckpt"

    python trainer_pl_all.py \
        --config-name=ICRA_Decoupled_DP_T_FILM_stage2 \
        seed=${SEED} \
        \
