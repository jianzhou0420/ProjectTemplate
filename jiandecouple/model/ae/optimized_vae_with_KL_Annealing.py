...
class VAE1D(nn.Module):
    ...
    def loss_function(self, *args, **kwargs) -> dict:
        ...
        if current_epoch is None:
            raise ValueError("KL Annealing requires 'current_epoch' to be passed to the loss function.")

        # 3. 动态计算 KLD Weight (KL Annealing 核心逻辑)
        if current_epoch < self.kl_annealing_start_epoch:
            kld_weight = 0.0
        elif current_epoch >= self.kl_annealing_end_epoch:
            kld_weight = 1.0
        else:
            # 线性增加
